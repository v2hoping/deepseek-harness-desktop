...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'settings.onboarding.credentialAction',
    kind: 'single',
    scope: 'root',
    summary: 'An additional way to satisfy the credential an onboarding step asks for, rendered beside its input.',
    doc: 'An additional way to satisfy the credential an onboarding step asks for,\nrendered beside its input. The step supplies the provider route so a\nregistrant can decline routes it has no path for; a surface that offers\nno such path leaves the seat empty and the step is unchanged.',
    registerOptions: [],
    ownerProps: [
